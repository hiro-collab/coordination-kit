...
- [ ] Directory responsibilities are clear for global ops files, team files, and reusable assets.
- [ ] `00_INDEX.md` exists where file growth is likely, or an equivalent index exists.
- [ ] A project spec file exists and defines mission scope and acceptance criteria.
- [ ] `SHARED_NOTE.md` exists.
- [ ] `INTEGRATION.md` exists.
- [ ] `WORKING_MEMORY.Commander.md` exists.
- [ ] `WORKING_MEMORY.A.Shoulder.md` exists.
- [ ] `LOG.Commander.md` exists.
- [ ] `LOG.A.Shoulder.md` exists.
- [ ] A public trial brief exists and is shared to both teams.
- [ ] The primary human language for judge-facing and player-facing guidance is fixed.
- [ ] `RUN_MONITOR.md` exists.
- [ ] One judging scorecard exists per competing team.
- [ ] `KIT_RETROSPECTIVE.md` exists or is queued for post-run use.

## Thread Setup

- [ ] Commander thread is active.
- [ ] A.Shoulder is a visible thread if direct user interaction is required.
- [ ] A.Shoulder has authority to spawn and steer fingers.
- [ ] Commander retains mission scope and final sign-off.
- [ ] Judge roles are explicit before execution starts.
- [ ] Required read-and-act instructions have been sent explicitly to the correct thread.
...
- [ ] Start/finish updates to `WORKING_MEMORY.<node>.md` and `LOG.<node>.md` are required.
- [ ] Each team has its own writable workspace.
- [ ] Cross-team visibility rules are written down.
...
- [ ] Each team is asked to predict what user-facing judging criteria are likely to matter most.
...
- [ ] Scope or fairness changes are applied equally across teams or rejected.
...
- [ ] A.Shoulder has provided acceptance evidence and residual gaps.
...
- [ ] Each team has a completed judging scorecard.
