...
2. Decide the exact kit path you will tell the AI, for example `../coordination-kit` or `./assets/coordination-kit`.
3. Keep live run files in the project workspace only. Do not let the AI write into the shared parent directory or back inside the kit.
4. Open both the project and the kit in a file-aware AI coding system such as Codex or Claude Code, or any similar tool that can read repository files and follow written instructions.
5. Tell the AI to use `coordination-kit` as the coordination method, name the kit path, and restrict writes to visible project directories such as `./run-ops` and `./run-workspaces`.
6. Confirm that the AI created visible ownership and sign-off structure before real implementation starts.
...
Treat `workspace/` only as a container.
Do not use it as a live run area.
...
- no live run files appearing in the shared parent directory
...
2. AI に伝える kit の実パスを先に決める。たとえば `../coordination-kit` か `./assets/coordination-kit`
3. live run file は project 側の workspace にだけ置き、共有の親階層や kit 本体には書かせない
4. project と kit の両方を file-aware な AI coding system で開く
5. `coordination-kit` を使うこと、kit の場所、`Commander` を立てること、Arm を 1 つ以上作ること、mission / working-memory / integration / verification / handoff の visible file を project 内に作ることを AI に伝える
6. 実装を始める前に、ownership と sign-off の構造が visible file として出ているか確認する
...
`workspace/` 直下は container として扱い、live run area にしません。
...
- shared な親階層に live run file が増えない
